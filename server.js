...
async function searchFlights(req, res) {
  ...
  try {
    ...
    const db = client.db(dbName);
    const collection = db.collection("rails");
    ...
    if (result.length > 0) {
      console.log("Retrieved matching Trains:", result);
      res.json(result);
    } else {
      console.log("No Trains found for the given criteria");
      res.status(404).send("No Trains found for the given criteria");
    }
  } catch (err) {
    console.error("Error:", err);
    res.status(500).send("An error occurred during Train search");
  } finally {
    await client.close();
    console.log("Disconnected from the database");
  }
